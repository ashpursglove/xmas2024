...
So in short I had to break it up into smaller parts as compressed archives to get round size limits and antivirus checks! So apologies for the ballache but its the only way I could get this to you on christmas short of rush DHL-ing you a USB!

So lets get this up and running on your laptop!! 

# Step 1 - 7-Zip
First, you're going to need 7-Zip, if you dont have it you can download it and install it here
...
If you know how to use GitHub then you know what youre doing so skip to step 3.
...
You can delete the readme.md file, its fine if you dont but probably better if you do.
...
if it doesent immedietly come up in the list of options click on the "show more options" part.
...
Annoyingly I had actually built the program to support switching between Curiosity, Oppertunity and Spirit as well as all the cameras but NASA have stopped the API for Oppertunity and Spirit! There are ways of web scraping the pics but thats a whole project in itself. Maybe an idea for version 2.0!
...
After you have selected which you'd prefer, pop in your preferred date/Sol. Obviously if youre wanting to look at the landing cams etc go with sol 0.

When you have everything ready click on the "Get Photos" button.

NOW THIS TAKES AGES!!! Dont worry, it hasnt crashed. The NASA API for other services like EPIC is fine, but for the mars rover the requests take sooooooooo long to get back so sit tight and wait with bated breath!
...
# EPIC Images
So EPIC is the Earth Polychromatic Imaging Camera. its positioned at the L1 point between the Earth and the Sun which basically means its always taking pictures of a fully lit earth.

Same deal with the inputs, put in a date or leave it blank for the most recent.

You can see in this picture as it is winter now, Australia is near centeral in the pic as it is summer there. try loading some pictures and cycling through during our summer times and look at the difference!
...
Here you can look up exactly whos currently in space! 
...
I've made it start with just the ISS and Hubble selected so its not a giant mess, but as you add more they will be added to the map and their courses for the next orbits mapped.
...
Interesting note, because GOES-16 (black) is in a geostationary orbit it doesn't have a orbital track over the ground, how cool is that!!


# Settings

Just a simple settings menu. During testing of sending this to other computers the size of the text turned out to be an issue so I've put a text size slider in there so you can suit it to wherever your running in on whayever screen.

I've also make it run a dark mode by default but I've also make another lighter colour scheme for everything if you prefer to be blinded so horses for courses. Just select the mode in the dropdown menu that you prefer.
...
#
As this is just for you, I've embedded the API keys into the binaries themselves. As its just your keys, there is a limit to how much you can use this..... turns out it's 1000 requests per hour! If for some reason you hit that limit you wil be blocked for 1 hour and then the key will begin to work again.
#
~~As a result of how I'm doing the memory stuff on the back end, please avoid changing tabs while a picture is loading. In most cases this is bearly an issue but with the long load times for the Mars rover pictures it becomes one. So try to avoid changing tabs whenever you can see a loading bar.~~
...
#
The satellite tracker and the astronaut list use a different API to the NASA one with different limits. The satellite tracker is similar in limit to the NASA one but the crew and craft look up one is smaller. I dont know exactly what it is but I have hit the limit during testing. If you do hit the limit just give it an hour to reset but you shouldnt be needing to spam that list anyway!
#
